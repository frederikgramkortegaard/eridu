...
int are_literals(struct AST_NODE * a, struct AST_NODE * b) { return is_literal(a) && is_literal(b); }
...
/* Contains:
    ...
    Constant Propagation ( Convert variables that aren't ever reassigned into literals )
    Intrinsic Functions  ( builtins )
    ...
    switch (parent->kind) {
            case A_ADD:
                ...
                if ((x < 0) && (a > INT_MAX + x) || (x > 0) && (a < INT_MIN + x)) {
                    printf("Warning! Possible integer (over)- underflow in addition at line '%d'\n", parent->lineno);
                    break;
                }

                parent->kind = A_INTVAL;
                parent->data.intval = parent->left->data.intval + parent->right->data.intval;
                break;

            case A_SUB:

                if ((x < 0) && (a > INT_MAX + x) || (x > 0) && (a < INT_MIN + x)) {
                    printf("Warning! Possible integer (over)- underflow in subtraction at line '%d'\n", parent->lineno);
                    break;
                }
            ...
            case A_MUL:
                ...
                if ((a == -1) && (x == INT_MIN) || (x == -1) && (a == INT_MIN) || (a > INT_MAX / x) || (a < INT_MIN / x)) {
                    printf("Warning! Possible integer (over)- underflow in multiplication at line '%d'\n", parent->lineno);
                    break;
                }
...
void fold_floats(struct AST_NODE * parent) {
    ...
    switch (parent->kind) {
            case A_ADD:
                ...
                parent->data.floatval = parent->left->data.floatval + parent->right->data.floatval;
                break;
...
void optimize_inner(struct AST_NODE * tree, struct AST_NODE * parent) {
    ...
    // Intrinsics (builtins)
    switch (tree->kind) {
        // We can reduce castable calls at compile time since we have static typechecking
        case A_CASTABLE:
            tree->kind = A_INTVAL;      
            
            tree->data.intval = castable(get_evaluation_type_of_subtree(tree->right), get_evaluation_type_of_subtree(tree->left));
            did_fold = 1;
            break;
        
        // Since we've already calculated the size of any type previously, we can just insert it as an intval
        case A_SIZEOF:
