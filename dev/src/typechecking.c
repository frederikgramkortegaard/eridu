...
int castable(struct TYPE *from, struct TYPE *to) {
  ...
  // @NOTE : Structures can't be cast to be another struct, even if they're 100%
  // equivalent
...
int are_equivalent_types(struct TYPE *a, struct TYPE *b) {
  ...
  } else if ((a->type == A_STRING && is_char_pointer(b)) ||
             (is_char_pointer(a) && b->type == A_STRING)) {
    return 1;
  }
...
int is_arithmetic_type(struct TYPE *a) {
  ...
  // @NOTE : Pointer arithmatic does NOT increment in the way you'd think.
  // adding 1 to a pointer increases it by a literal one, and not by an address
  case A_POINTER:
    return 1;
...
int is_compareable(struct TYPE *a) {
  // Double pointers are essentially arrays, which are not comparable in and of
  // themselves, except for '==' and '!=' which are handled by the caller of
  // this function.
...
void typecheck_parameter_lists(struct AST_NODE *caller,
                               struct TYPE *caller_type, struct TYPE *callee) {
  if (caller_type == NULL && callee == NULL) {
    return;

    // For untyped callables we infer the return type from the LHS and let the
    // programmer worry about the safety
  ...
  if (caller_type->number_of_params != callee->number_of_params) {
    fprintf(stderr,
            ...
            caller_type->id, callee->number_of_params,
            caller_type->number_of_params, caller->lineno);
    exit(-1);
  }
  ...
  for (int i = 0; i < caller_type->number_of_params; i++) {
    ...
    if (are_equivalent_types(a, b) == 0) {
      if (a->type == A_CALLABLE) {
        fprintf(stderr,
                "types.c :: typecheck_parameter_lists :: Function call '%s' "
                "was given a CALLABLE argument with "
                "signature:\n",
                caller_type->id);

        print_type(a, 1);
        fprintf(stderr,
                "where the signature of the function being called is:\n");
        print_type(b, 1);
        ...
        fprintf(stderr,
                "types.c :: typecheck_parameter_lists :: Function call '%s' "
                "expected a struct of type %s with ID %s, "
                "but was given type:\n",
                caller_type->id, ast_node_kinds_as_strings[a->pointer->type],
                a->pointer->id);
        ...
        fprintf(
            ...
            "line '%d'\n",
            caller_type->id, ast_node_kinds_as_strings[a->type],
            ast_node_kinds_as_strings[b->type], caller->lineno);
      }
      exit(-1);
    }
  }
...
// array
struct TYPE *
fetch_evaluated_parameter_types_from_subtree(struct AST_NODE *node,
                                             struct TYPE *caller_type) {
  if (node == NULL) {
    return NULL;
  }
...
struct TYPE *get_evaluation_type_of_subtree(struct AST_NODE *node) {
  ...
  case A_MOD:
    ...
    // the callable to be that of the left-hand side, and let the programmer
    // worry about the rest
    if (b == NULL) {
      b = a;

      printf("WARNING! Infering return type '%s' from untyped callable at line "
             "'%d'\n",
    ...
    if (are_equivalent_types(a, b) == 0 || is_arithmetic_type(a) == 0) {
      fprintf(stderr,
              ...
              ast_node_kinds_as_strings[a->type],
              ast_node_kinds_as_strings[b->type],
  ...
  case A_GREAT:
    ...
    if (are_equivalent_types(a, b) == 0 ||
        (is_compareable(a) == 0 && node->kind != A_EQUAL &&
         node->kind != A_NEQUAL)) {
  ...
  case A_ASSIGN:
    ...
    if (are_equivalent_types(a, b) == 0) {
      ...
      if (node->right->kind == A_ALLOCATE) {
        fprintf(stderr,
                "Remember, allocate returns a pointer to the type given as the "
                "first parameter at line '%d'\n",
                node->lineno);
      }

      fprintf(stderr, "Remember, if you're working with pointer pointers the "
                      "nesting will not be show in the output of the "
                      "expected types\n");
      exit(-1);
    }
  ...
  // @WARN : Calling a callable that is not actually aliasing a properly defined
  ...
  case A_CALL_FUNC:
    ...
    // Check if the given parameters match the expected parameters as defined in
    // the respective A_DEF_FUNC node
  ...
  case A_CAST:
    ...
    if (castable(get_evaluation_type_of_subtree(node->right),
                 get_evaluation_type_of_subtree(node->left)) != 1) {
      fprintf(
          stderr,
          "types.c :: get_evaluation_type_of_subtree :: Illegal Cast - tried "
          "to cast from type '%s' to type '%s' at line '%d'\n",
          ast_node_kinds_as_strings[get_evaluation_type_of_subtree(node->right)
                                        ->type],
          ast_node_kinds_as_strings[get_evaluation_type_of_subtree(node->left)
                                        ->type],
          node->lineno);
      exit(-1);
    }
  ...
  case A_ALLOCATE:
    ...
    if (b->type != A_INT) {
      fprintf(stderr,
              "types.c :: get_evaluation_type_of_subtree :: the builtin "
              "'allocate' takes in and integer-expression, "
  ...
  case A_RETURN:
    ...
    if (are_equivalent_types(
            get_evaluation_type_of_subtree(node->left),
            fetch_type_from_subtree(function_stack[function_stack_cursor - 1])
                ->return_type) != 1) {
      fprintf(
          stderr,
          ...
          ast_node_kinds_as_strings[fetch_type_from_subtree(
                                        function_stack[function_stack_cursor -
                                                       1])
                                        ->return_type->type],
          ast_node_kinds_as_strings[get_evaluation_type_of_subtree(node->left)
                                        ->type],
  ...
  case A_CONTINUE:
  case A_BREAK:
    if (loop_stack_cursor == 0 ||
        node->symbol_table != loop_stack[loop_stack_cursor - 1]->symbol_table) {
      fprintf(stderr,
  ...
  case A_INDEX:
    ...
    if (b->type != A_INT) {
      fprintf(stderr,
              ...
              "Only integer values can be used to index into an array at line "
              "'%d'\n",
              node->lineno);
      exit(-1);
    }

    // Only arrays can be indexed, so we have to check whether or not the ID
    // we're trying to index into is at least a double pointer
    // @NOTE : We only have dynamic arrays, so [] is never used for anything but
    // indexing.
    ...
    // If we're trying to index into something that isn't a pointer or a string
    // its an error
    if (is_deep_pointer(callback->type) == 0) {
      fprintf(stderr,
              "types.c :: get_evaluation_type_of_subtree :: Cannot index into "
              "a non-nested or non-string variable at "
              "line '%d'\n",
              node->lineno);
      exit(-1);
    }
  ...
  case A_DOT:
#if DEBUG
    printf("types.c :: get_evaluation_type_of_subtree :: A DOT node was "
           "encountered at line '%d'\n",
           node->lineno);
#endif
    ...
    if (a->type != A_POINTER || a->pointer->type != A_STRUCT) {
      fprintf(stderr,
              "types.c :: get_evaluation_type_of_subtree :: Cannot access a "
              "member of a non-struct at line '%d'\n",
              node->lineno);
      exit(-1);
    }
    ...
    if (b == NULL) {
      fprintf(stderr,
              "types.c :: get_evaluation_type_of_subtree :: Struct '%s' does "
              "not have member '%s' at line '%d'\n",
  ...
  case A_STRUCT:
    ...
    if (callback == NULL) {
      fprintf(stderr,
              "types.c :: get_evaluation_type_of_subtree :: Struct declaration "
              "'%s' could not find itself at line '%d'\n",
              node->left->id, node->lineno);
      exit(-1);
    }
  ...
  case A_PRINT:
    ...
    if (a == NULL) {
      fprintf(stderr,
              "Cannot print return value of untyped callable, you need to "
              "unpack it first at line '%d'\n",
              node->lineno);
      exit(-1);
    };
    return a;
