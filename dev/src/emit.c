/* Uses the CODE_GENERATION_STATUS struct from codegen.c to recursivly generate assembly code
   corrosponding to the intermidiate form */
...
char * intermediate_to_x86(struct INSTRUCTION * instruction, struct CODE_GENERATION_STATUS * status) {
...
/* Format a codegen.c :: ARGUMENT struct into assembly compatible arguments wrt. spacing, commas' etc.*/
void format_arg(struct ARGUMENT * arg, int comma, struct CODE_GENERATION_STATUS * status) {
    if (arg == NULL) {
        return;
    }
        ...
        case REG:
            if (arg->target->register_number == 1) {
                ...
                if (arg->mode->mode == INDIRECT) {
                    ...
                    asprintf(&tmp, " %d(%r%d) ", arg->mode->offset, arg->target->register_number);
                    ins(status, tmp, func_cursor);
                    break;
                } 
                asprintf(&tmp, " %r%d ", arg->target->register_number);
...
/* Pushes a 0 onto the current stack position as represented by the %rsp register.
 * Factored into its own function as to make the code more self-documenting.*/
...
/* Convert a simple codegen.c :: INSTRUCTION struct into valid assemvly instructions */
void simple_instruction(struct INSTRUCTION * instruction, struct CODE_GENERATION_STATUS * status) {
...
/* It is easier to describe divsion strictly rather than through codegen and simple instructions */
...
void ins(struct CODE_GENERATION_STATUS * status, char * str, int cursor) {
    code_sections[cursor] =
        realloc(code_sections[cursor], strlen(code_sections[cursor]) + strlen(str) + 1);
    strcat(code_sections[cursor], str);
}

void malloc_setup(struct CODE_GENERATION_STATUS * status) {

    asprintf(&tmp, "movq %rsi , %r14 # Saving potential cmd line arguments before malloc\n");
    ins(status, tmp, func_cursor);

    asprintf(&tmp, "movq $4000 , %rdi # Moving size of initial allocation into RDI\n");
    ins(status, tmp, func_cursor);

    asprintf(&tmp, "callq malloc # calling C-binary for allocating stackspace on multiple platforms\n");
    ins(status, tmp, func_cursor);

    asprintf(&tmp, "movq %rax , %rdx # Moving heap pointer into RSL\n");
    ins(status, tmp, func_cursor);

    asprintf(&tmp, "movq %r14 , %rsi # Restoring potential cmd line arguments after malloc\n");
    ins(status, tmp, func_cursor);
}

void emit_asm(struct CODE_GENERATION_STATUS * status, struct FLOAT_DATA ** float_data_array) {
    hash_stack[0] = 0;
    ...
    while (float_data_array[i] != NULL) {
        asprintf(&tmp, "%s: .float %f\n", float_data_array[i]->label, float_data_array[i]->floatval);
        ins(status, tmp, func_cursor);
        i++;
    }
    i = 0;
    while (status->string_data_array[i] != NULL) {
        asprintf(&tmp, "%s: .string %s\n", status->string_data_array[i]->label, status->string_data_array[i]->stringval);
        ins(status, tmp, func_cursor);
        i++;
    }
    ...
    for (int i = 0; i < status->instructions_length; i++) {
        ...
        switch (status->instruction_array[i]->operator) {
            ...
            case O_FUNC_DEF:
                ...
                // Each function is stored in-memory as different char arrays, here we assign a given function a specific index
                // in the list of char arrays based on the hash of the function ID
                func_cursor = crc32b_hash_emit(status->instruction_array[i + 1]->arg1->target->label);
...
#if DEBUG
                        fprintf(stderr,
                                "emit.c :: emi_asm :: Too many functions defined, function '%s' could not be given a code "
