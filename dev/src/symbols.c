...
// Dynamically allocate space for params and append them to the given types list
// of parameters
void push_param_to_type_params(struct TYPE *type, struct TYPE *param) {
  if (type == NULL || param == NULL) {
    fprintf(
        stderr,
        "Error: push_param_to_type_params() called with NULL type or param\n");
    exit(1);
  }

  type->params = realloc(type->params,
                         sizeof(struct TYPE *) * (type->number_of_params + 1));
...
#endif
      fprintf(stderr,
              "Error: struct '%s' is not defined in any accessible scope at "
              "line '%d'\n",
...
SYMBOL *make_symbol(char *id, struct TYPE *type,
...
int get_symbol_offset_in_stackframe(char *id, struct SYMBOL_TABLE *table) {
  ...
  if (symbol_category->category == S_PARAM) {
    for (int i = 0;
         i < table->number_of_parameters + table->number_of_variables; i++) {
  ...
  for (int i = 0; i < table->number_of_parameters + table->number_of_variables;
       i++) {
    if (strcmp(table->symbols[i]->id, id) == 0) {
      break;
    }
...
#if DEBUG
  printf("symbols.c :: get_symbol_offset_in_stackframe :: getting offset '%d' "
         "for symbol '%s' by traversing the given symbol table and keeping "
         "count of the position in which the symbol is found\n",
         offset, id);
#endif
...
#if DEBUG
  printf("symbols.c :: lookup_in_scope :: Trying to lookup ID '%s' in current "
         "scope\n",
         id);
#endif
...
#if DEBUG
  printf("symbols.c :: lookup :: Trying to lookup ID '%s' in given scope and "
         "all parent scopes\n",
         id);
#endif
...
int insert(char *id, struct TYPE *type, enum SYMBOL_CATEGORY category,
  ...
  if (table->symbol_table_size ==
      table->number_of_variables + table->number_of_parameters) {
    table->symbol_table_size *= 2;
    table->symbols =
        realloc(table->symbols, sizeof(SYMBOL) * table->symbol_table_size);
  }
  ...
  if (inserted == 0) {
    fprintf(stderr,
            "symbols.c :: insert :: No available slots in symbol table, the "
            "maximum possible symbol count is '%d'\n",
            SYMBOL_TABLE_SIZE);
    exit(-1);
  }
#ifdef DEBUG
...
#if DEBUG
    printf(
        "Error: push_member_to_field() called with NULL member or to-type\n");
#endif
    fprintf(
        stderr,
        "Could not add struct member to struct, this is a compiler error.\n");
    exit(1);
  }

  to->fields =
      realloc(to->fields, sizeof(struct TYPE *) * (to->number_of_fields + 1));
  to->fields[to->number_of_fields] = member;
  to->number_of_fields++;
}

// Propagate a symbol-table through a subtree, giving all nodes in the subtree
// the given table as its node->symbol_table
...
void fill_struct_type(struct AST_NODE *node, struct TYPE *type) {
  ...
  if (node->kind == A_PARAM) {
    if (node->left->kind == A_STRUCT) {
      fprintf(stderr,
              "Cannot declare a struct member with type of a struct without it "
              "being a pointer at line '%d'\n",
              node->lineno);
      exit(-1);
    }

    // @NOTE : This isn't super efficient, since we're traversing the sub-tree
    // once more. we should really propagate this information during an earlier
    // pass
    ...
    if (res != NULL) {
      fprintf(stderr,
              "Error: struct '%s' already has a member '%s' at line '%d'\n",
              type->id, node->right->id, node->lineno);
      exit(-1);
    }
...
int expand_ast_with_symbol_tables(struct AST_NODE *tree,
  ...
  if (tree->kind == A_BLOCK || tree->kind == A_PROGRAM) {
    ...
    if (tree->left->kind == A_STRUCT) {
      fprintf(stderr,
              "Cannot declare a variable as a struct, only as a pointer to a "
              "struct, at line '%d'\n",
              tree->lineno);
      exit(-1);
    }
    ...
    if (contains_return(tree) == 0) {
      fprintf(stderr,
              "Function '%s' does not contain a return statement, which is "
              "required at line '%d'\n",
...
#if DEBUG
    printf("symbols.c :: extend_ast_with_symbol_tables :: Beginning to parse a "
           "function definition\n");
#endif
