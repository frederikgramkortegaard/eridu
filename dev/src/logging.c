 ...
 * this code is not meant to be pretty, rather it has a purely functional purpose. */
...
int count_nodes(struct AST_NODE * tree) {
    ...
    int nodes = 1 + count_nodes(tree->left) + count_nodes(tree->mid) + count_nodes(tree->right);
    return nodes;
}